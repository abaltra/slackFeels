...
passport.connect = function (req, query, profile, next) {
  ...
  if (profile.hasOwnProperty('_json')) {
    user.username = profile._json.user;
    user.user_id = profile._json.user_id;
  }
    ...
    if (!req.user) {
      ...
      if (!passport) {
        User.create(user, function (err, user) {
          ...
          // TODO
          // EmailService.userWelcome({
          //   email: user.email
          // });

          // TODO refactor
          if (profile.hasOwnProperty('_json')) {
            var team_id = profile._json.team_id;
            var team_name = profile._json.team;
            Team.findOne({
              team_id: team_id
            }, function(err, team){
              if(err){
